...
export default function AccediForm({ t }: { t: Dict["auth"] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (!res.ok) {
      setError(res.error ?? t.erroreAccesso);
      return;
    }
    router.push("/studio");
    router.refresh();
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-paper2 px-6 py-14">
      <div className="w-full max-w-lg rounded-[28px] bg-white p-10 shadow-sm">
        ...
        <p className="mt-4 text-center text-lg text-muted">{t.accediIntro}</p>
